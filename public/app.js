(() => {
  ...
  var showLoader = () => document.getElementById("global-loader").classList.remove("hidden");
  var hideLoader = () => document.getElementById("global-loader").classList.add("hidden");
  ...
  var deleteManual = (id) => fetchWithAuth(`/api/manuals/${id}`, {
    method: "DELETE"
  });

  // public/js/utils/lazy-loader.js
  var lazyLoad = (componentPath) => {
  ...
  var componentRegistry = {
    ...
    "AppMain": "/js/app.js"
    // Add main app for lazy loading if needed
  };
...
(See https://redux.js.org/faq/organizing-state#can-i-put-functions-promises-or-other-non-serializable-items-in-my-store-state)`);
  ...
  // public/js/store/slices/hvacSlice.js
  var initialState = {
    selectedSystem: null,
    diagnosticSteps: [],
    currentStep: 0,
  ...
  var hvacSlice_default = hvacSlice.reducer;

  // public/js/store/slices/authSlice.js
  var initialState2 = {
    user: null,
    token: null,
    refreshToken: null,
  ...
  // public/js/store/slices/uiSlice.js
  var initialState3 = {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (desktopSearch) {
      desktopSearch.addEventListener("input", () => displayManuals2());
    }
    if (mobileSearch) {
      mobileSearch.addEventListener("input", () => displayManuals2());
    }
    if (categoryFilter) {
      categoryFilter.addEventListener("change", displayManuals2);
    }
    if (kbSearchInput) {
    ...
    if (manageManualSearch) {
      manageManualSearch.addEventListener("input", displayManuals2);
    }
    if (manageCategoryFilter) {
      manageCategoryFilter.addEventListener("change", displayManuals2);
    }
    ...
    function displaySearchResults(results) {
      ...
      if (!container) return;
      container.innerHTML = results.length ? results.map((result) => `<div class="result-item">${result.title}</div>`).join("") : "<p>No results found.</p>";
    }
    async function displayManuals2() {
      try {
